...
struct DEV_RgbLED : Service::LightBulb {       // RGB LED (Command Cathode)
  ...
  DEV_RgbLED(int red_pin, int green_pin, int blue_pin) : Service::LightBulb(){       // constructor() method
    ...
    strip = Adafruit_NeoPixel(3, MOSI, NEO_GRB + NEO_KHZ800);
  ...
  boolean update(){                         // update() method
    ...
    for (uint16_t i = 0; i < strip.numPixels(); i++) {
        strip.setPixelColor(i, strip.ColorHSV(h * 65535UL / 360, s * 255 / 100, v * 255 / 100));
    } 
    strip.show();
...
struct DEV_HeatFan : Service::HeaterCooler {

    SpanCharacteristic *active;
    SpanCharacteristic *currentTemp;
    SpanCharacteristic *currentState;
    SpanCharacteristic *targetState;
    SpanCharacteristic *heatThresh;

    DEV_HeatFan() : Service::HeaterCooler() {
        active = new Characteristic::Active();
        currentTemp = new Characteristic::CurrentTemperature(30);
        currentState = new Characteristic::CurrentHeaterCoolerState();
        targetState = new Characteristic::TargetHeaterCoolerState();
        heatThresh = new Characteristic::HeatingThresholdTemperature(40);
    }
